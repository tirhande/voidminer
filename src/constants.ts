 ...
 * 업그레이드 비용.
 *
 * GDD 07 에서 재료는 현재 티어 광물 + 고급 광물 + 화폐로 확정됐다. 이 프로토는
 * "고급 광물"을 보석으로 읽는다. 상위 티어 광물로 읽으면 그 광물을 캐기 위해
 * 업그레이드가 필요한데 업그레이드에 그 광물이 필요해져 순환이 생긴다.
 */
export const UPGRADE_COST = {
  /** 첫 단계에 드는 현재 티어 주괴 수 */
  IngotBase: 6,
  /** 단계가 오를 때마다 더해지는 주괴 수 */
  IngotPerLevel: 4,
  /** 첫 단계에 드는 보석 수 */
  GemBase: 1,
  /** 단계가 오를 때마다 더해지는 보석 수 */
  GemPerLevel: 1,
  /** 첫 단계에 드는 화폐 */
  CreditBase: 40,
  /** 단계가 오를 때마다 더해지는 화폐 */
 ...
 * 거점 매각가.
 *
 * GDD 03 은 주 수입원을 광물이 아닌 것으로 두라고 제안한다 — 광물을 팔아야만
 * 진행되면 "팔까 쓸까"가 갈등이 아니라 강요가 되기 때문이다. 이 프로토에는
 * 전투가 없으므로 보석이 그 자리를 맡고, 광물은 팔 수는 있되 값이 낮다.
 */
